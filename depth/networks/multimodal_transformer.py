...
class Multimodal_SelfAttention(nn.Module):
    def __init__(self , config):
        ...
        self.w_vis = nn.Parameter(torch.tensor(0.5))
        self.w_imu = nn.Parameter(torch.tensor(0.5))
    ...
    def forward(self, hidden_state , attention_mask ):
        ...
        attention_prob_vis = nn.Softmax(-1)(attention_score[:,:,:,:self.vis_len])
        attention_prob_imu = nn.Softmax(-1)(attention_score[:,:,:,self.vis_len:self.vis_len+self.imu_len])
        attention_prob = torch.cat((attention_prob_vis,attention_prob_imu),dim=-1)
        ...
        #===================================================================
        m_vis = self.w_vis * torch.ones(1,1,156,145).to(DEVICE)#132 268 156
        m_imu = self.w_imu * torch.ones(1,1,156,11).to(DEVICE)#132 268 156
        # ===================================================================
        mask2 = torch.cat((m_vis,m_imu),dim=3).to(DEVICE)
