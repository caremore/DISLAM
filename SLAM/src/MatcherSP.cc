...
namespace ORB_SLAM3
{
    ...
    const float MatcherSP::TH_HIGH = 0.75;
    const float MatcherSP::TH_LOW = 0.6;

    MatcherSP::MatcherSP()
    {
    }

    int MatcherSP::SearchByProjection(Frame &F, const vector<MapPoint*> &vpMapPoints, float fNNratio, const float th, const bool bFarPoints, const float thFarPoints)
    {
    ...
    float MatcherSP::RadiusByViewingCos(const float &viewCos)
    {
        if(viewCos>0.998)
            return 2.5;
        else
    ...
    int MatcherSP::SearchByBoW(KeyFrame* pKF,Frame &F, vector<MapPoint*> &vpMapPointMatches)
    {
        /**
    * @brief 使用词袋模型进行关键帧与当前帧的特征匹配
    * @param pKF 关键帧（参考帧）
    * @param F 当前帧（待匹配帧）
    * @param vpMapPointMatches 输出匹配的地图点
    * @return 成功匹配的数量
    * 
    * 用途：重定位和局部地图跟踪时，快速建立关键帧与当前帧的数据关联
    * 特点：使用暴力匹配替代词袋加速，适用于SuperPoint特征
    */    
    // ==================== 数据准备 ====================
        // 获取关键帧中的所有地图点匹配
        ...
        * @brief 使用L2距离的暴力匹配器
        * 
        * cv::NORM_L2: 使用欧氏距离（适用于SuperPoint的浮点描述子）
        * true: 启用交叉检查（Cross Check），提高匹配质量
        * 
        * 注意：原版ORB使用汉明距离(cv::NORM_HAMMING)，这里使用L2适应SuperPoint
        */
        ...
        /**
        * @brief 收集关键帧中有效地图点的索引
        * 
        * 只处理那些有有效地图点且地图点状态良好的特征点
        * 跳过空指针和坏的地图点
        */
        vector<int> vRealIndexKF;
        vRealIndexKF.reserve(vpMapPointsKF.size()); // 预分配内存
        ...
        * @brief 执行暴力匹配
        * 
        * 比较关键帧描述子(realDescriptorsKF)和当前帧描述子(F.mDescriptors)
        * 为每个关键帧描述子在当前帧中寻找最佳匹配
        */
        ...
        /**
        * @brief 遍历所有匹配对，应用距离阈值筛选
        * 
        * 只保留描述子距离小于TH_LOW的匹配对
        * 这些匹配被认为是高质量的匹配
        */
        for (auto &match : matches)
        {
            ...
            if (match.distance < TH_LOW)
            {
                ...
                * @brief 建立匹配关系映射
                * 
                * match.queryIdx: 在realDescriptorsKF中的索引（对应vRealIndexKF）
                * match.trainIdx: 在当前帧F中的特征点索引
                */
                ...
                * @brief 建立地图点关联
                * 
                * 将关键帧中的地图点关联到当前帧的对应特征点
                * 这样当前帧的特征点就拥有了对应的3D地图点信息
                */
    ...
    int MatcherSP::SearchByProjection(KeyFrame* pKF, Sophus::Sim3f &Scw, const vector<MapPoint*> &vpPoints,
    ...
    int MatcherSP::SearchByProjection(KeyFrame* pKF, Sophus::Sim3<float> &Scw, const std::vector<MapPoint*> &vpPoints, const std::vector<KeyFrame*> &vpPointsKFs,
    ...
    /**
    * @brief 初始化阶段的特征匹配（用于单目SLAM初始化）
    * @param F1 第一帧（参考帧）
    * @param F2 第二帧（当前帧）
    * @param vbPrevMatched 先前匹配点位置（输入输出参数）
    * @param vnMatches12 输出匹配关系：F1特征点索引 -> F2特征点索引
    * @param fNNratio 最近邻比率阈值，用于排除模糊匹配
    * @param windowSize 搜索窗口大小（像素）
    * @return 成功匹配的数量
    * 
    * 用途：单目SLAM系统初始化阶段，在两帧间寻找足够的特征匹配来初始化地图
    * 特点：只使用原始尺度的特征点，确保匹配的稳定性
    */
    int MatcherSP::SearchForInitialization(Frame &F1, Frame &F2, vector<cv::Point2f> &vbPrevMatched, vector<int> &vnMatches12, float fNNratio, int windowSize)
    {
        int nmatches=0;// 匹配计数器
        // 初始化匹配关系：F1中每个特征点在F2中的匹配索引，初始为-1表示无匹配 
        vnMatches12 = vector<int>(F1.mvKeysUn.size(),-1);

        // 记录F2中每个特征点的最佳匹配距离，用于避免重复匹配
    ...
    int MatcherSP::SearchByBoW(KeyFrame *pKF1, KeyFrame *pKF2, vector<MapPoint *> &vpMatches12)
    {
...
*/
    ...
    int MatcherSP::SearchForTriangulation(KeyFrame *pKF1, KeyFrame *pKF2,
                                           vector<pair<size_t, size_t> > &vMatchedPairs, const bool bOnlyStereo, const bool bCoarse, std::vector<float> *vfDebugInfo) const
    {
        const vector<MapPoint*> vpMapPoints1 = pKF1->GetMapPointMatches();
        const cv::Mat &Descriptors1 = pKF1->mDescriptors;
...
*/
    int MatcherSP::Fuse(KeyFrame *pKF, const vector<MapPoint *> &vpMapPoints, const float th, const bool bRight)
    {
    ...
    int MatcherSP::Fuse(KeyFrame *pKF, Sophus::Sim3f &Scw, const vector<MapPoint *> &vpPoints, float th, vector<MapPoint *> &vpReplacePoint)
    {
        // Get Calibration Parameters for later projection
        const float &fx = pKF->fx;
        const float &fy = pKF->fy;
    ...
    int MatcherSP::SearchBySim3(KeyFrame* pKF1, KeyFrame* pKF2, std::vector<MapPoint *> &vpMatches12, const Sophus::Sim3f &S12, const float th)
    {
    ...
    int MatcherSP::SearchByProjection(Frame &CurrentFrame, const Frame &LastFrame, const float th, const bool bMono)
    {
        int nmatches = 0;
    ...
    int MatcherSP::SearchByProjection(Frame &CurrentFrame, KeyFrame *pKF, const set<MapPoint*> &sAlreadyFound, const float th , const float threshold)
    {
        int nmatches = 0;
    ...
    int MatcherSP::FilterMatchesByResponces(const KeyFrame* pKF, std::vector<MapPoint*>& vpMatches, float fGoodRes, int nMinMatches)
    {
        int nCurNum = 0;
        if (nMinMatches == -1)
        {
    ...
    void MatcherSP::ComputeThreeMaxima(vector<int>* histo, const int L, int &ind1, int &ind2, int &ind3)
    {
        int max1=0;
        int max2=0;
        int max3=0;
    ...
    float MatcherSP::DescriptorDistance(const cv::Mat &a, const cv::Mat &b)
    {
