...
KeyFrame::KeyFrame():
        ...
        mnTrackReferenceForFrame(0), mnFuseTargetForKF(0), mnBALocalForKF(0), mnBAFixedForKF(0), mnBALocalForMerge(0), mnBAGlobalForKF(0),
        fx(0), fy(0), cx(0), cy(0), invfx(0), invfy(0), 
        mnRelocQuery(0), mRelocScore(0), mRelocAccScore(0), mnPlaceRecognitionQuery(0), mPlaceRecognitionScore(0), mPlaceRecognitionAccScore(0),
...
KeyFrame::KeyFrame(Frame &F, Map *pMap, KeyFrameDatabase *pKFDB):
    ...
    mnBAGlobalForKF(0), mnRelocQuery(0), mRelocScore(0), mRelocAccScore(0), mnPlaceRecognitionQuery(0), mPlaceRecognitionScore(0), mPlaceRecognitionAccScore(0),
    fx(F.fx), fy(F.fy), cx(F.cx), cy(F.cy), invfx(F.invfx), invfy(F.invfy),
    mbf(F.mbf), mb(F.mb), mThDepth(F.mThDepth), N(F.N), mvKeys(F.mvKeys), mvKeysUn(F.mvKeysUn),
    mvuRight(F.mvuRight), mvDepth(F.mvDepth), mDescriptors(F.mDescriptors.clone()), mGlobalDescriptors(F.mGlobalDescriptors.clone()),
    ...
    mvKeysRight(F.mvKeysRight), NLeft(F.Nleft), NRight(F.Nright), mTrl(F.GetRelativePoseTrl()), mnNumberOfOpt(0), mbHasVelocity(false), imgLeft(F.imgLeft),mpSPextractorLeft(F.mpSPextractorLeft) // For Debug
{
    mnId=nNextId++;
