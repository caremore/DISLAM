...
public:
    ...
    Frame(const cv::Mat &imLeft, const cv::Mat &imRight, const double &timeStamp, SuperPointExtractor* extractorLeft, SuperPointExtractor* extractorRight, cv::Mat &K, cv::Mat &distCoef, const float &bf, const float &thDepth, GeometricCamera* pCamera,Frame* pPrevF = static_cast<Frame*>(NULL), const IMU::Calib &ImuCalib = IMU::Calib());

    // Constructor for RGB-D cameras.
    Frame(const cv::Mat &imGray, const cv::Mat &imDepth, const double &timeStamp, SuperPointExtractor* extractor, cv::Mat &K, cv::Mat &distCoef, const float &bf, const float &thDepth, GeometricCamera* pCamera,Frame* pPrevF = static_cast<Frame*>(NULL), const IMU::Calib &ImuCalib = IMU::Calib());

    // Constructor for Monocular cameras.
    Frame(const cv::Mat &imGray, const double &timeStamp, SuperPointExtractor* extractor, GeometricCamera* pCamera, cv::Mat &distCoef, const float &bf, const float &thDepth, Frame* pPrevF = static_cast<Frame*>(NULL), const IMU::Calib &ImuCalib = IMU::Calib());
    ...
    //SP
    void ExtractSPPoints(int flag, const cv::Mat &im, const int x0, const int x1);
...
public:
    ...
    SuperPointExtractor* mpSPextractorLeft, *mpSPextractorRight;
    ...
    //SuperPoint容器 （只用于前端跟踪）
    //std::vector<cv::KeyPoint> mvKeysSP, mvKeysRightSP;
    //std::vector<cv::KeyPoint> mvKeysUnSP;

    //cv::Mat mDescriptorsSP,mDescriptorsRightSP;
